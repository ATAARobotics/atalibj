...
public abstract class Bindable implements BindableAxises, BindableButtons {
    ...
    /**
     * Removes all binds for buttons on a port.
     *
     * @param port port to remove binds from
     */
    public final void removeButtonBinds(int port) {
        ...
        while (e.hasMoreElements()) {
            BindKey key = (BindKey) e.nextElement();
            if (key.port == port && !key.type.equals(AXIS)) {
                bindings.remove(key);
                pressed.remove(key);
            }
        }
    ...
    private void doBind(BindKey key) {
        ...
        if (type.equals(WHEN_PRESSED)) {
            doWhenPressed(key, (Command) bind);
        } else if (type.equals(WHILE_PRESSED)) {
            doWhilePressed(key, (Command) bind);
        } else if (type.equals(WHEN_RELEASED)) {
            doWhenReleased(key, (Command) bind);
        } else if (type.equals(WHILE_RELEASED)) {
            doWhileReleased(key, (Command) bind);
        } else if (type.equals(AXIS)) {
            doAxisBind(key, (AxisBind) bind);
        }
    }

    private void doWhenPressed(BindKey key, Command Command) {
    ...
    private void doWhilePressed(BindKey key, Command Command) {
        if (getPressed(key.port)) {
            Command.run();
        }
    }

    private void doWhenReleased(BindKey key, Command Command) {
    ...
    private void doWhileReleased(BindKey key, Command Command) {
        if (!getPressed(key.port)) {
            Command.run();
        }
    }
